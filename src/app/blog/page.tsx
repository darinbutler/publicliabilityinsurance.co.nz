...
export default function BlogPage() {
  ...
  return (
    <>
      {/* Hero Section */}
      <section
        className="relative min-h-[380px] flex items-end"
        style={{
        ...
        <section className="max-w-6xl mx-auto px-4 py-16">
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {blogPosts.map((post) => (
              <article
                key={post.slug}
                className="bg-white rounded-xl overflow-hidden border-2 border-slate-200 hover:shadow-xl hover:border-emerald-500 transition-all duration-300"
              >
                {/* Post Image */}
                <div
                  className="w-full h-48"
                  style={{
                    backgroundImage: post.image ? `url(${post.image})` : 'url(https://images.unsplash.com/photo-1450101499163-c8848c66ca85?auto=format&fit=crop&w=800&q=80)',
                    backgroundSize: 'cover',
                    backgroundPosition: 'center',
                  }}
                />
